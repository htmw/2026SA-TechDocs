...
export async function POST(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	if (daily_log.getHungerEvent(parsed.data.occurred_at)) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	const updated = await daily_log.updateHungerEvent(parsed.data.occurred_at, parsed.data as any);
	if (!updated) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	const updated_event = await dailyLog.updateHungerEvent(parsed.data.occurred_at, parsed.data as any);
	if (updated_event) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<Params> }) {
    ...
    const deleted = await dailyLog.deleteHungerEvent(parsed.data.occurred_at);
	if (!deleted) {
		return NextResponse.json(createErrorResponse("HUNGER_EVENT_NOT_FOUND", "No matching hunger event found"), { status: 404 });
	}
