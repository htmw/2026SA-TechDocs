...
export async function GET(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	const payload = daily_log.getCravingEvent(new Types.ObjectId(id));
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	const updated = await daily_log.updateCravingEvent(new Types.ObjectId(id), parsed.data as any);
	if (!updated) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<Params> }) {
	...
	const updated_event = await dailyLog.updateCravingEvent(new Types.ObjectId(id), parsed.data as any);
	if (updated_event) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<Params> }) {
    ...
    const deleted = await dailyLog.deleteCravingEvent(new Types.ObjectId(id));
	if (!deleted) {
		return NextResponse.json(createErrorResponse("CRAVING_EVENT_NOT_FOUND", "No matching craving event found"), { status: 404 });
	}
